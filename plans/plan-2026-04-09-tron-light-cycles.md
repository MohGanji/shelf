...
| **P10.186** | Tron `index.html`: JSON-LD `WebSite` adds `isAccessibleForFree` (`true`, aligned with `VideoGame` / `WebPage`) so the site entity exposes the same free-access CreativeWork signal as the main entities in `@graph` | P10.185 | [x] |
...
| **P10.198** | Tron `index.html`: JSON-LD `WebSite` adds `contentRating` (`Everyone`, aligned with `WebPage.contentRating` and `VideoGame.contentRating`) so the site entity exposes the same simple audience rating label in `@graph` alongside `isFamilyFriendly`, `genre`, and `keywords` | P10.197 | [x] |
...
| **P10.203** | Tron `index.html`: JSON-LD each `PlayAction` and nested `EntryPoint` `target` add `inLanguage` (`en`, aligned with `WebPage` / `WebSite` / `VideoGame`) so structured data marks the play action and entry point as English alongside `EntryPoint` URL and platform metadata | P10.202 | [x] |
| **P10.204** | Tron `index.html`: JSON-LD each `PlayAction` adds `actionStatus` (`https://schema.org/PotentialActionStatus`) so structured data marks the browser play affordance as an available potential action alongside `name`, `description`, `inLanguage`, and `EntryPoint` `target` | P10.203 | [x] |
| **P10.205** | Tron `index.html`: JSON-LD each `PlayAction` (`WebSite`, `WebPage`, `VideoGame`/`WebApplication`) adds `object` (`{ "@id": "./#game" }`) so structured data explicitly links the play affordance to the same `VideoGame` / `WebApplication` entity as `mainEntity`, `about`, and `offers.itemOffered` | P10.204 | [x] |
| **P10.206** | Tron `index.html`: JSON-LD each `PlayAction` (`WebSite`, `WebPage`, `VideoGame`/`WebApplication`) adds `agent` (`{ "@id": "./#organization" }`) so structured data identifies the facilitating publisher for the browser play affordance alongside `object`, `actionStatus`, and `EntryPoint` metadata | P10.205 | [x] |
...
| **P10.222** | Tron `index.html`: JSON-LD `VideoGame` / `WebApplication` adds `creditText` (plain-text attribution aligned with `Organization.name` / `publisher`) so structured data exposes explicit credits alongside `abstract`, `copyrightNotice`, and `copyrightHolder` | P10.221 | [x] |
...
| **P10.237** | Tron `index.html`: JSON-LD `VideoGame` / `WebApplication` adds `softwareHelp` as `CreativeWork` (`name`, `inLanguage` `en`, plain-text keyboard/quick-start `text`) so structured data exposes embedded help documentation alongside `accessibilitySummary`, `browserRequirements`, and `featureList` | P10.236 | [x] |
...
| **P10.251** | Tron `index.html`: JSON-LD standalone `Offer` `./#offer-tlc-free` adds `hasMerchantReturnPolicy` as `MerchantReturnPolicy` (`returnPolicyCategory` `MerchantReturnNotPermitted`, `merchantReturnDays` `0`, plain-text `name`/`description`, `inLanguage` `en`) so structured data states that no merchandise return process applies to the zero-price browser-delivered grant alongside `shippingDetails` and `availableDeliveryMethod` | P10.250 | [x] |
| **P10.252** | Tron `index.html`: JSON-LD `@graph` adds standalone `MerchantReturnPolicy` `./#merchant-return-policy-tlc` (same fields as P10.251); `./#offer-tlc-free` `hasMerchantReturnPolicy` becomes `{ "@id": "./#merchant-return-policy-tlc" }` so return metadata is a deduplicated graph node alongside `./#offer-tlc-free` (pattern from P10.238) | P10.251 | [x] |
| **P10.253** | Tron `index.html`: JSON-LD `@graph` adds standalone `OfferShippingDetails` `./#offer-shipping-tlc-free` (same fields as the nested block under `./#offer-tlc-free` in P10.242); `./#offer-tlc-free` `shippingDetails` becomes `{ "@id": "./#offer-shipping-tlc-free" }` so shipping metadata is a deduplicated graph node alongside `./#merchant-return-policy-tlc` and `./#offer-tlc-free` (pattern from P10.252) | P10.252 | [x] |
| **P10.254** | Tron `index.html`: JSON-LD `@graph` adds standalone `Place` `./#place-worldwide` and `DefinedRegion` `./#defined-region-worldwide` (`name` `Worldwide` each); `Organization.areaServed`, `./#offer-tlc-free` `eligibleRegion` + `areaServed`, `VideoGame.spatialCoverage`, and `./#offer-shipping-tlc-free` `shippingDestination` reference those `@id`s so worldwide geography deduplicates across entities (extends P10.168, P10.246, P10.219, P10.253) | P10.253 | [x] |
| **P10.255** | Tron `index.html`: JSON-LD `@graph` adds standalone `MonetaryAmount` `./#monetary-amount-zero-usd` (`currency` `USD`, `value` 0); `./#offer-shipping-tlc-free` `shippingRate` references that `@id` so zero-dollar shipping money deduplicates like `Place` / `DefinedRegion` / `OfferShippingDetails` from P10.253–P10.254 | P10.254 | [x] |
| **P10.256** | Tron `index.html`: JSON-LD `@graph` adds standalone `QuantitativeValue` `./#quantitative-value-zero-sec-range` (`minValue`/`maxValue` `0`, `unitCode` `SEC`); `./#offer-shipping-tlc-free` `ShippingDeliveryTime` `handlingTime` and `transitTime` reference that `@id` so identical zero-second duration nodes deduplicate alongside `./#monetary-amount-zero-usd` | P10.255 | [x] |
