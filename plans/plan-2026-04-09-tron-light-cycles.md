...
- **Missing:** trail derez wiring (P2.3), AI, gates, interior barrier merge (P5.5), power-ups, portals, garage UI upgrades (P7.4), editor, full HUD beyond speed/nitro/trail count, most SFX/music content, Phase 6–7 UI flows.
- **P7.3:** `js/ui/garageShowroom.js` — dark void + grid floor, emissive plate, `createLightCycle` with save colors, short emissive trail preview, turntable rotation; `mountGarageDestinationScreen` uses top chrome overlay (`tron-destination--garage`) over live canvas.
- **P4.1 / P4.2:** `js/game/enemies.js` — campaign enemies from `level.enemies[]` (per-enemy `getArenaPlaytestConfig`, intelligence 1–10), cycle mesh + trail `ownerId` `enemy-*`, physics bodies; frozen until first W in non-lobby. **`js/game/ai.js`:** `computeEnemyCycleKeys` — multi-map `hasTrailAhead` (player + each enemy trail, querier `selfId`), `raycastSolidClearanceXZ` (arena AABB + barriers); seek player with trail/wall avoidance. Tick passes player body + trail; `aiSteer` drives cycle tilt.
